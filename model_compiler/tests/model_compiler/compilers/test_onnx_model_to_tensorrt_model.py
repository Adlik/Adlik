...
class ConfigTestCase(TestCase):
    def test_from_json(self):
        self.assertEqual(TensorrtConfig(max_batch_size=7, enable_fp16=False, enable_strict_types=False),
                         TensorrtConfig.from_json({'max_batch_size': 7}))

        self.assertEqual(TensorrtConfig(max_batch_size=7, enable_fp16=False, enable_strict_types=False),
                         TensorrtConfig.from_json({'max_batch_size': 7, 'enable_fp16': False}))

        self.assertEqual(TensorrtConfig(max_batch_size=7, enable_fp16=True, enable_strict_types=False),
                         TensorrtConfig.from_json({'max_batch_size': 7, 'enable_fp16': True}))

        self.assertEqual(TensorrtConfig(max_batch_size=7, enable_fp16=False, enable_strict_types=True),
    ...
    def test_from_env(self):
        ...
        self.assertEqual(TensorrtConfig(max_batch_size=7, enable_fp16=False, enable_strict_types=False),
                         TensorrtConfig.from_env({'MAX_BATCH_SIZE': '7', 'ENABLE_FP16': '0'}))

        self.assertEqual(TensorrtConfig(max_batch_size=7, enable_fp16=True, enable_strict_types=False),
                         TensorrtConfig.from_env({'MAX_BATCH_SIZE': '7', 'ENABLE_FP16': '1'}))
    ...
    def test_compile_int8_default_calibrator(self):
        with TemporaryDirectory() as file_path:
            ...
            resnet50_onnx_model = onnx_compiler.compile_source(source=ONNXModelFile(resnet50_onnx_model_file),
                                                               config=OnnxConfig(input_formats=
            ...
            compiled = compiler.compile_source(source=resnet50_onnx_model,
                                               config=TensorrtConfig(max_batch_size=1, enable_int8=True,
