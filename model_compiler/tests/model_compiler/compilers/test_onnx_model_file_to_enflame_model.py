    ...
    def test_from_json(self):
        self.assertEqual(
            Config.from_json({
                ...
                'resource_mode': 'base_resource',
                'build_flag': 'TopsInference.KFP16'
            }),
...
class CompileSourceTestCase(TestCase):
    ...
    def test_compile_with_variables(self):
        with NamedTemporaryFile(suffix='.onnx') as model_file:
            ...
            compiled = compiler.compile_source(source=ONNXModelFile(
                model_file.name),
                                               config=config)

        self.assertEqual(
            [model_input.name for model_input in compiled.model_inputs],
            ['x:0'])
