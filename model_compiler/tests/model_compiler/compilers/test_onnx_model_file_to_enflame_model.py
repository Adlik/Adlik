...
import model_compiler.compilers.onnx_model_file_to_enflame_model as onnx_file_to_enflame_compiler
...
class CompileSourceTestCase(TestCase):
    def test_compile_with_variables(self):
        with NamedTemporaryFile(suffix='.onnx') as model_file:
            ...
            compiled = onnx_file_to_enflame_compiler.compile_source(source=ONNXModelFile(model_file.name), config=config)
