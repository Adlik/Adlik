...
class TorchscriptModel(NamedTuple):
    config: Config
    outputs: torch.Tensor    # pylint: disable=no-member
    ...
    def get_script_outputs(self) -> Sequence[ModelOutput]:
        script_outputs = []
        for i, output in enumerate(self.outputs):
            script_outputs.append(ModelOutput(name='output_'+str(i) if output.name is None else str(output.name),
                                  data_type=DataType.from_torch_data_type(self.config.data_type).to_tf_data_type(),
