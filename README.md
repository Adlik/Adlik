...
We provide an automatic [benchmark tool](https://github.com/Adlik/Adlik/tree/master/benchmark) to test the inference performance of different DL models with Adlik on the same CPU or GPU, and using the benchmark tool we give the test performance data of the simple CNN model (MNIST model), the ResNet50 model, and InceptionV3 with different serving engines.
