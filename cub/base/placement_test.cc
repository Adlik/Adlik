...
FIXTURE(PlacementTest1){TEST("should new object on placement memory"){Placement<Student> studentMemory;
...
FIXTURE(PlacementTest2){TEST("should delay init the member object in placement"){Object object;
...
private:
  Energy* create(WorkerType type) {
    switch (type) {
      case HUMAN:
        return new (m.human.alloc()) HumanEnergy();
      case ROBOT:
        return new (m.robot.alloc()) ChargeEnergy();
      default:
        return nullptr;
    }
  }
...
FIXTURE(PlacementTest3){TEST("should cast to the public role correctly for human"){WorkerObject human(HUMAN);
