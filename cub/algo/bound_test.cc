...
FIXTURE(UpperBoundTest){
    TEST("should find the upper bound in middle position correct"){ASSERT_THAT(upper_bound(a, ARR_SIZE(a), 4), eq(2));
}
...
FIXTURE(LowerBoundTest){
    TEST("should find the upper bound in middle position correct"){ASSERT_THAT(lower_bound(a, ARR_SIZE(a), 4), eq(1));
}
