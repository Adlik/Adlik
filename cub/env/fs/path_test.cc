...
FIXTURE(PathTest){TEST("dir name: fullpath, path is empty"){
    ASSERT_EQ(StringView("hdfs://127.0.0.1:9000"), Path("hdfs://127.0.0.1:9000").dirName());
}  // namespace

TEST("dir name: fullpath, path is /data") {
  ASSERT_EQ(StringView("hdfs://127.0.0.1:9000/data"), Path("hdfs://127.0.0.1:9000/data/train.csv.tfrecords").dirName());
}

TEST("dir name: fullpath, path is root /") {
  ASSERT_EQ(StringView("hdfs://127.0.0.1:9000/"), Path("hdfs://127.0.0.1:9000/train.csv.tfrecords").dirName());
}
...
TEST("join paths") {
  ...
  ASSERT_EQ("/foo/bar/baz/blah/blink/biz"s, paths("/foo/bar/baz/", "/blah/blink/biz"));
