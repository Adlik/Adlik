...
FIXTURE(UriTest){
    ...
    void assertUri(const std::string& str, StringView scheme, StringView host, StringView path){Uri uri(str);
...
TEST("no host") {
  ...
  assertUri("local.file:///usr/local/foo", "local.file", "", "/usr/local/foo");
}

TEST("full path") {
  assertUri("hdfs://localhost:8020/path/to/file", "hdfs", "localhost:8020", "/path/to/file");
  assertUri("hdfs://localhost:8020/", "hdfs", "localhost:8020", "/");
}
}
;
