...
bool parseScheme(StringView& remain, StringView& scheme) {
  return Scanner(remain).one(alpha()).many(is_or(alnum(), dot())).stopCapture().literal("://").result(&remain, &scheme);
}
...
void parse(StringView remain, StringView& scheme, StringView& host, StringView& path) {
...
Uri::Uri(StringView scheme, StringView host, StringView path) : scheme(scheme), host(host), path(path) {
}
