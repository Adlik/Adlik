...
def _save_tensorflow_model(model_path):
    with tf.Graph().as_default(), tf.compat.v1.Session(graph=tf.Graph()) as sess:
        ...
        tf.compat.v1.train.Saver().save(sess, model_path)


class ConfigTestCase(TestCase):
    def test_from_json(self):
        self.assertEqual(Config.from_json({'input_names': ['input'], 'output_names': ['output'],
                                           'input_formats': ['channels_first']}),
                         Config(input_info=[('input', DataFormat.CHANNELS_FIRST)], output_names=['output']))

    def test_from_env(self):
        self.assertEqual(Config.from_env({'INPUT_NAMES': 'input1,input2:0', 'OUTPUT_NAMES': 'output',
                                          'INPUT_FORMATS': 'channels_first'}),
...
class CompileSourceTestCase(TestCase):
    ...
    def test_compile_with_op_name(self):
        with TemporaryDirectory() as directory:
            model_path = os.path.join(directory, 'model.ckpt')
            _save_tensorflow_model(model_path)
            config = Config.from_json({'input_names': ['x', 'y'], 'output_names': ['z'],
